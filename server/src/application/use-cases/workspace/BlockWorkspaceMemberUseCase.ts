...
export class BlockWorkspaceMemberUseCase {
    ...
    async execute(workspaceId: string, ownerId: string, targetUserId: string): Promise<WorkspaceMember> {
        ...
        if (ownerId === targetUserId) {
            throw new AppError("Cannot block the owner of the workspace", HttpStatusCode.BAD_REQUEST);
        }
        ...
        const updatedMember = await this.workspaceMemberRepository.updateStatus(workspaceId, targetUserId, 'blocked');
        if (!updatedMember) {
            throw new AppError("Failed to block member", HttpStatusCode.INTERNAL_SERVER);
        }
